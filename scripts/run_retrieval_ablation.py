...
def _metrics_row(name: str, metrics: RetrievalMetrics) -> str:
    """Render one retriever's metrics as a Markdown table row."""
    return (
        f"| {name} | {metrics.recall_at_k:.3f} | {metrics.precision_at_k:.3f} "
        f"| {metrics.hit_rate_at_k:.3f} | {metrics.mrr:.3f} "
...
def _render_report(
    ...
    *,
    ...
    lines = [
        ...
        f"- **Fusion:** Reciprocal Rank Fusion (k_rrf={DEFAULT_K_RRF})",
        "",
        f"## Metrics @{k}",
        "",
    ...
    lines += [_metrics_row(name, metrics) for name, metrics in results.items()]
    lines.append("")
    return "\n".join(lines)
